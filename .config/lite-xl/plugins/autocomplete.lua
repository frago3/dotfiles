...
local function draw_suggestions_box(av)
  ...
  renderer.draw_rect(rx, ry, rw, rh -25 ,style.background3)
  ...
  for i=start_index, start_index+show_count-1, 1 do
    ...
    local color = (i == suggestions_idx) and style.line_number2 or style.line_number
    common.draw_text(font, color, s.text, "left", rx + style.padding.x, y, rw, lh)
    -- if s.info then
    --   color = (i == suggestions_idx) and style.text or style.dim
    --   common.draw_text(font, color, s.info, "right", rx, y, rw - style.padding.x, lh)
    -- end
    y = y + lh
  ...
  -- renderer.draw_rect(rx, y, rw, 2, style.background)
